...
ScalingGraphicsView::~ScalingGraphicsView()
{
...
ScalingGraphicsView::ScalingGraphicsView(QWidget *parent): QGraphicsView(parent)
{
...
void ScalingGraphicsView::wheelEvent ( QWheelEvent * event )
{
    ...
    {//按住ctrl键 可以放大缩小
        ...
        else if((event->delta() < 0)&&(scale_m <= 0.1))//图像缩小到自适应大小之后就不继续缩小
        {
